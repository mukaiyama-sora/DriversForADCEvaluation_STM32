...
{
	app.Init();
	UARTDriver::WriteLine(std::bitset<16>(123).to_string());
}
...
void Application::Rreg(const Args& args)
{
	...
	uint64_t out = 0;
	uint64_t shift = 32;
	for(const auto& reg : spi_driver_.GetBuffer()) {
...
void Application::Radc(const Args& args)
{
	...
	size_t record_length = static_cast<size_t>(std::stol(args.front()));
	this->record_.resize(record_length);

	HAL_NVIC_EnableIRQ(EXTI15_10_IRQn);
	while(SPIDriver::GetReadCount() < record_length);

	for(const auto& record : this->record_) {
		UARTDriver::WriteLine(std::bitset<32>(record).to_string());
	}
}

extern "C" void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if(const size_t count = SPIDriver::GetReadCount(); count != 0 && (count - 1) < app.record_.size()) {
		const auto& buffer = SPIDriver::GetBuffer();
		const auto data =
